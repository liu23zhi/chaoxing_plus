...
const RATE_COMPARISON_EPSILON = 0.001;
...
export class Panel {
  ...
  updateSettings(settings: Settings): void {
    this.settings = settings;
    if (!this.el) return;
    const speedVal = this.el.querySelector<HTMLSelectElement>('#cx-speed');
    if (speedVal) speedVal.value = String(settings.playbackRate);
    this.syncCheckboxes();
  }
    ...
    return `
      ...
      <div class="cx-panel-body">
        ...
        <div class="cx-row">
          ...
          <select id="cx-speed">
            ${this.buildSpeedOptions(s.playbackRate)}
          </select>
        </div>
        <div class="cx-row">
  ...
  private buildSpeedOptions(currentRate: number): string {
    const speedOptions = ['1', '1.25', '1.5', '1.75', '2', '2.5', '3', '4', '6', '8', '12', '16'];
    return speedOptions
      .map((rate) => {
        const selected = Math.abs(parseFloat(rate) - currentRate) < RATE_COMPARISON_EPSILON ? ' selected' : '';
        return `<option value="${rate}"${selected}>${rate}x</option>`;
      })
      .join('');
  }
  ...
  private collectAndSave(): void {
    ...
    const num = (id: string) => parseFloat((get<HTMLInputElement | HTMLSelectElement>(id)?.value) ?? '1');
