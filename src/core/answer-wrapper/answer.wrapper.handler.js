...
export async function defaultAnswerWrapperHandler(answererWrappers, env) {
    const searchInfos = [];
    const temp = JSON.parse(JSON.stringify(answererWrappers));
    if (temp.length === 0) {
        throw new Error('题库配置不能为空，请配置后重新开始自动答题。');
    }
    await Promise.all(temp.map(async (wrapper) => {
        const { name = '未知题库', homepage = '#', method = 'get', type = 'fetch', contentType = 'json', headers = {}, data: wrapperData = {}, handler = 'return (res)=> [JSON.stringify(res), undefined]' } = wrapper;
        try {
            let results = [];
            let requestData;
            let url;
            if (method.toLocaleLowerCase() === 'get') {
                ...
                Object.keys(wrapperData).forEach((key) => {
                    if (typeof wrapperData[key] === 'object' && Reflect.has(wrapperData[key], 'handler')) {
                        const dynamicHandler = Function(Reflect.get(wrapperData[key], 'handler'))();
            ...
            if (info && Array.isArray(info)) {
                if (info.every((item) => Array.isArray(item))) {
                    results = results.concat(info.map((item) => ({
                        question: item[0],
                        answer: item[1],
                        extra_data: item[2] || {}
                    })));
